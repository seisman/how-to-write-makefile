...
“.PHONY : all” 不写的话一般情况也可以正确的执行，但是原理是因为all作为目标文件但项目中并没有，
所以make就会希望得到他的依赖，执行后面的命令从而得到all目标，即使因为并没有得到all文件的相关命
令所以并不会得到all目标。由此实现了类似伪目标的效果。但是如果目录中真的有一个all文件的话就会被告
知“make: 对all无需做任何事”。建议：显式写出是一个好习惯。）
