...
“隐含规则”也就是一种惯例，make会按照这种“惯例”心照不宣地来运行，那怕我们的Makefile中没有书写
...
使用“模式规则”会显得更智能和清楚，但“后缀规则”可以用来保证我们Makefile的兼容性。
...
#. 编译C++程序的隐含规则。
   ...
   ``<n>.o`` 的目标的依赖目标会自动推导为 ``<n>.cc`` 或 ``<n>.cpp`` 或是 ``<n>.C`` ，并且其生成命令是
   ``$(CXX) –c $(CPPFLAGS) $(CXXFLAGS)`` 。（建议使用 ``.cc`` 或 ``.cpp`` 作为C++源文件的后缀，而不是 ``.C`` ）
...
#. 汇编和汇编预处理的隐含规则。
   ...
   默认使用C预编译器 ``cpp`` ，并且其生成命令是： ``$(CPP) $(CPPFLAGS)`` 。
...
如果 ``%`` 定义在目标中，那么，依赖中的 ``%`` 的值决定了目标中的 ``%`` 的值，也就是说，
依赖中的模式的 ``%`` 决定了目标中 ``%`` 的样子。例如有一个模式规则如下：
